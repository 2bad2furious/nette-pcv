...
class HeaderManagerOld extends Manager implements IHeaderManager {
    const TABLE = "header",
        COLUMN_ID = "header_id",
        COLUMN_LANG = "lang",
    ...
    /**
     * @param HeaderPageOld $header
     * @param Language $language
     * @throws Exception
     */
    private function addChildren(HeaderPageOld &$header, Language $language): void {
     ...
     */
    public function getRoot(Language $language, ?Page $currentPage): HeaderPageOld {
    ...
    /**
     * @param int $id
     * @return HeaderPageOld|null
     */
    public function getById(int $id):?HeaderPageOld {
    ...
    public function addPage(HeaderPageOld $parent, int $pageId, string $title): int {
        ...
        dump($langId, $parent, $page);
        if (!$page instanceof Page) throw new InvalidArgumentException("Page $pageId with lang $langId not found");
    ...
    public function addCustom(HeaderPageOld $parent, string $title, string $url): int {
    ...
    public function editPage(HeaderPageOld $headerPage, int $pageId, string $title) {
    ...
    public function editCustom(HeaderPageOld $headerPage, string $title, string $url) {
        if (!filter_var($url, FILTER_VALIDATE_URL)) throw new InvalidArgumentException("Url not valid");
    ...
    public function delete(int $id) {
        ...
        if (!$headerPage instanceof HeaderPageOld) throw new InvalidArgumentException("Header page $id not found");
    ...
    private function setPagesOnChildren(HeaderPageOld &$parent, ?Page $currentPage, Language $language): bool {
        $isActive = false;
    ...
    private function getHeaderFromDb(Language $language) {
        $headerPage = new HeaderPageOld(0, null, $language->getId(), null, null, 0);
        $this->addChildren($headerPage, $language);
        return $headerPage;
    }

    private function getFromRow(IRow $row) {
        return new HeaderPageOld(
            $row[self::COLUMN_ID],
